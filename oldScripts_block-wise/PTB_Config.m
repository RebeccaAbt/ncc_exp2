classdef PTB_Config < o_ptb.base.Config
    ...
    methods (Access = public)
        ...
        % ------------------------------------------------------------------- v
        function MRI_compatible(obj, do_it) % to adjust screen settings to fit for MRI screen

            if do_it

                fprintf('\n making screen options MRI suitable \n ')

                obj.fullscreen = true;
                obj.crappy_screen = false;
                SetResolution(0,1920,1080,120)

                fprintf('\n defined resolution inside "MRI_PC" in PTB Config\n')
        ...
        % --------------------------------------------------------------- V
        function is_MRIcompatible = MRIlab_conditions(obj)

            fprintf('\ncheck if it is MRI suited in MRIlab_conditions in PTB_Config\n')

            screenSettings = Screen('Resolution', 0);
