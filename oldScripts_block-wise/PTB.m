classdef (Sealed) PTB < handle
    ...
    methods (Static, Access = public)
        function ptb = get_instance(ptb_config)
            ...
            if nargin < 1
                ...
                % ------------------------------------------------------- v
                if o_ptb.PTB.is_MRI_monitor
                    if ~ptb_config.MRIlab_conditions()
                        fprintf('Screen setting will be adjusted because you are in the MRI');
                        ptb_config.adjust_for_MRI = true;
                    end
                end
                % ------------------------------------------------------- ^
    ...
    methods (Access = public)
        function setup_screen(obj)
            ...
            if obj.ptb_config.adjust_for_MRI
                %                 screen_resolution = SetResolution(0,1920,1080,120)
                screen_resolution = SetResolution(0,1920,1080,60);
                window_resolution = [screen_resolution.width screen_resolution.height];
                fprintf('adjust_for_MRI in PTB line 204')
        ...
        function setup_trigger(obj)
            ...
            try
                p = pyenv;
                has_python = p.Version ~= '';
            catch
            end

            %       try
            %         [version,executable, isLoaded] = pyversion;
            %         has_python = ~isempty(version) && isLoaded;
            %       catch
            %       end
            ...
            if ~isa(trigger_system, 'function_handle')
                if obj.has_datapixx()
                    ...
                    fprintf('\n --->  using Datapixx as trigger system \n\n');
                elseif has_python && labjack.Labjack.has_labjack
                    trigger_system = @o_ptb.subsystems.trigger.Labjack;
                    fprintf('\n --->  using LabJack as trigger system \n\n');
                else
        ...
        function setup_tactile(obj)
            ...
            if ~isa(tactile_system, 'function_handle')
                if o_ptb.subsystems.tactile.CorticalMetricsStimulator.is_present
                    ...
                    fprintf('\n --->  using CorticalMetrics as tactile system \n\n');
                else
                    tactile_system = @o_ptb.subsystems.tactile.Dummy;
                    fprintf('\n --->  using Dummy as tactile system \n\n');
                end %if
            end %if
        ...
        %--------------------------------------------------------------------------
        function triggered = get_trigger(obj, chan_nr, chan_type, timeout)

            triggered = 0;

            % Wait for external Trigger.
            %
            % Wait for the LabJack device to receive an external trigger.
            %
            % Parameters
            % ==========
            % chan_nr : integer
            %   The channel number to monitor for the trigger. Valid ranges are [0-7] for FIO/EIO
            %   and [0-4] for CIO.
            %
            % chan_type : char, optional
            %   The type of channel to monitor. Valid options are 'FIO', 'EIO', or 'CIO'.
            %   The default is 'FIO'.
            ...
            triggered = obj.trigger_subsystem.wait_for_intrigger(chan_nr, chan_type, timeout);

        end
        %--------------------------------------------------------------------------

        %--------------------------------------------------------------------------
        function triggered = trigger_status(obj, chan_nr, chan_type)
            if nargin < 3
                chan_type = 'FIO';
            end

            triggered = obj.trigger_subsystem.intrigger_status(chan_nr, chan_type);
        end
        %--------------------------------------------------------------------------
        ...
        function data = get_eyetracker_data(obj)
            % Read and return the current data from the eyetracker.
            %
            % If you have a Trackpixx, you should do this quite often because
            % it resets the memory used by the eyetracker.
            %
            % Returns
            % =======
            %
            % data : matrix
            %   Matrix with the current eye tracker data.

            data = obj.eyetracker_subsystem.get_data();
        end %function
    ...
    methods (Access = public, Static)
        function lab_pc = is_lab_pc()
            ...
            %
            % Whether it is a lab PC is determined by the `O_PTB_IS_LAB_PC`
            % environment variable.
            %
            % Returns
            % =======
            % bool
            %   ``true`` if the this is a lab stimulus PC.
            %
        ...
        %     end %static protected methods % ended here before "is_MRI_monitor" was added"

        % ----------------------------------------------------------------------- v
        function MRI_monitor = is_MRI_monitor()
       
            [status, thisMonitor] = system('powershell "Get-CimInstance -Query ''SELECT * FROM Win32_DesktopMonitor'' | Select-Object -ExpandProperty PNPDeviceID"');

%             if contains(thisMonitor, 'DISPLAY\CMRA320\5&10462A1A&0&UID4354') % MRI monitor
            if contains(thisMonitor, 'DISPLAY\LEN403A\4&39E3DC8C&0&UID8388688') % my Notebook Monitor
                MRI_monitor = true;
            end % if
        end %function
    ...
    methods (Access = private)
        function obj = PTB(ptb_config)
            ...
            if ~license('test', 'signal_toolbox')
                error('o_ptb needs the Signal Processing Toolbox');
            end %if
            try
                ...
                error('Something is wrong with your Psychtoolbox installation.\nMaybe you forgot to install gstreamer?\nAlso remember to run SetupPsychtoolbox at least once.\nThis is the error message: %s', e.message);
            end %try
