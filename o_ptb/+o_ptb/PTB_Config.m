classdef PTB_Config < o_ptb.base.Config
    ...
    methods (Access = public)
        function obj = PTB_Config()
            obj@o_ptb.base.Config();
            ...
            %         has_python = ~isempty(version);
            %       catch
            %       end
            
            if has_python
                obj.labjacktrigger_config = o_ptb.PTB_subconfigs.LabjackTriggers;
            else
                obj.dont_check_properties{end+1} = 'labjacktrigger_config';
            end %if
        ...
        % ---------------------------------------------------------------------- v
        function real_labjack_experiment(obj, do_it)
            % Shortcut for experiment configuration when using LabJack
            if do_it
                obj.fullscreen = true;
                obj.flip_horizontal = false;
                obj.hide_mouse = true;
                obj.skip_sync_test = false;
                obj.force_datapixx = false;
                obj.disable_datapixx = true;
                obj.crappy_screen = false;
                obj.internal_config.trigger_subsystem = @o_ptb.subsystems.trigger.Labjack;
                obj.internal_config.tactile_subsystem = @o_ptb.subsystems.tactile.CorticalMetricsStimulator;
                obj.labjacktrigger_config.method = labjack.Labjack.TriggerMethod.SINGLE;    %  TriggerMethod -->('SINGLE', 0, 'MULTI', 1, 'PULSEWIDTH', 2);
                obj.labjacktrigger_config.channel_group = labjack.Labjack.ChannelGroup.EIO;  %    ChannelGroup -->('FIO', 0, 'EIO', 1, 'CIO', 2);
            end %if
        end %function
        % ---------------------------------------------------------------------- ^
        ...
        function is_real = real_experiment_settings(obj)
            ...
            this_is_also_real = obj.fullscreen & ...
                ~obj.flip_horizontal & ...
                obj.hide_mouse & ...
                ~obj.skip_sync_test & ...
                ~obj.force_datapixx & ...
                ~obj.crappy_screen &...
                isequal(obj.internal_config.trigger_subsystem, @o_ptb.subsystems.trigger.Labjack) &...
                isequal(obj.internal_config.tactile_subsystem, @o_ptb.subsystems.tactile.CorticalMetricsStimulator)&...
                isequal(obj.labjacktrigger_config.channel_group, labjack.Labjack.ChannelGroup.EIO);
            
            is_real = this_is_real | this_is_also_real;          
        end %function
    end %methods
    ...
    methods (Access = protected)
        function cpObj = copyElement(obj)
            ...
            if ~isempty(obj.labjacktrigger_config)
                cpObj.labjacktrigger_config = copy(obj.labjacktrigger_config);
            else
                cpObj.dont_check_properties{end+1} = 'labjacktrigger_config';
            end %if
