function [params, ptb]=InitializeExp(outDir, params, ptb)
...
for iBlock = 1:length(modalities)
    ...
    params.(modality).trialorder2 = repmat([1; 2; 3; 4], 12, 1); % Repeat each stimulus 12 times
        
    if params.isTest
        params.(modality).trialorder3 = ones(params.nTrials, 1) * 2; % All trials same category
    else
        params.(modality).trialorder3 = ones(48, 1); 
    end
