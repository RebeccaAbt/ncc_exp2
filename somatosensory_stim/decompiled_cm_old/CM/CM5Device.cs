  ...
  internal class CM5Device
  {
    ...
    public CM5Device(int vendorid, int productid)
    {
      ...
      if (this.device == null)
      {
        ...
        {
          if (device.get_PID() == productid)
          {
            if (device.get_VID() == vendorid)
            {
              try
              {
      ...
      if (this.device.get_Descriptor().get_VID() != vendorid || this.device.get_Descriptor().get_PID() != productid)
      {
        this.device.Dispose();
        this.device = (USBDevice) null;
        throw new Exception("No Device Found");
      }
      this.Interface = this.device.get_Interfaces().get_Item(0);
      this.StartPolling();
    }
    ...
    {
      this.Interface.get_OutPipe().Write(bytes);
    }

    public byte[] ReceiveInputReport()
    {
      byte[] numArray = new byte[this.Interface.get_InPipe().get_MaximumPacketSize()];
      this.Interface.get_InPipe().Read(numArray);
      return numArray;
    }
    ...
    public byte[] ReceiveInputReport(int timeout, ref int cancel)
    {
      ...
      IAsyncResult asyncResult = this.Interface.get_InPipe().BeginRead(numArray, 0, numArray.Length, (AsyncCallback) (res => {}), new object());
      if (timeout == -1)
      {
        while (cancel < 0)
        {
      ...
      this.Interface.get_InPipe().Abort();
      return (byte[]) null;
    }
    ...
    private void StopPolling()
    {
      ...
      this.Interface.get_InPipe().Flush();
    }
  }
}
